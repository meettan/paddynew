...
class Transactions extends MX_Controller {
   ...
   //  Developed on 16/12/2020 after Food Api integration to delete Single Value // 
    ...
    public function f_mill_wise_gunnybag_upload(){
        ...
        if($_SERVER['REQUEST_METHOD'] == "POST") {
               ...
               //For Excel Upload
                ...
                if(!empty($_FILES['f_procurement_detail']['name']) && in_array($_FILES['f_procurement_detail']['type'],$csvMimes)){
                    ...
                    $totqty = 0;
                        ...
                        while(($line = fgetcsv($csvFile)) !== FALSE){
                            if($line[2] !='') {
                                ...
                                "rice_mill_name"   =>  $line[1],
                                "dispatch_qty"      =>  $line[2],
                                "receive_to_miller" =>  $line[3],
                                'resultant_cmr'    =>  $line[4],
                                "offer_qty"        =>  $line[5],
                                'offer_received'   =>  $line[6],
                                "do_issued"        =>  $line[7],
                                'do_deliver_cp'    =>  $line[8],
                                "do_deliver_sp"    =>  $line[9],
                                'do_deliver_tot'   =>  $line[10],
                                'cmr_pending'      =>  $line[11],
                                "paddy_in_hand"    =>  $line[12],
                                'gunny_bag'        =>  $line[13],
                                "kms_id"           =>  $this->session->userdata['loggedin']['kms_id'],
                                'created_by'       => $this->session->userdata['loggedin']['user_name'],
                                "created_dt"       =>  date('Y-m-d h:i:s')
                    ...
                    $this->Paddy->f_insert_multiple('td_mill_dis_gunny_upload', $data);
                ...
                $this->load->view('post_login/main');
                $this->load->view("mill_gunnybag/upload_fle");
                $this->load->view('post_login/footer');
        ...
        public function mill_wise_gunnybag(){ 

            $select  = array('a.*','b.branch_name');
            $where   = array('a.branch_id =b.id' => NULL);
            $data['rro_dtls'] =   $this->Paddy->f_get_particulars('td_mill_dis_gunny_upload a,md_branch b',$select,$where, 0);
            $this->load->view('post_login/main');
            $this->load->view("mill_gunnybag/dashboard",$data);
            $this->load->view('post_login/footer');

        }
        public function f_mill_wise_gunnybag_delete(){
